 ...
 */
export function cleanUserJid(jid: string): string {
...
export const warnCommand: Command = {
  ...
  execute: async (sock, msg, args) => {
    ...
    // Auto-expire manual warnings 30 days after they were issued (MongoDB TTL index, created once).
    if (!warnTtlIndexReady) {
      warnTtlIndexReady = true;
  ...
  execute: async (sock, msg, args) => {
    ...
    if (!isAdmin) {
      await sendHumanLikeResponse(
        sock,
        jid,
        { text: '❌ *Access Denied:* Only group admins can remove warnings.' },
        { quoted: msg }
      );
      return;
    }
...
export const checkWarnCommand: Command = {
  ...
  execute: async (sock, msg, args) => {
    ...
    text += `📝 *Message warnings:* ${count}/3\n`;
    for (let i = 0; i < count; i++) {
      text += `  ${i + 1}. ${warnings[i].reason}\n`;
    }
    ...
    await sendHumanLikeResponse(
      sock,
      jid,
      { text: text.trim(), mentions: [canonicalTargetJid] },
      { quoted: msg }
    );
  },
};
